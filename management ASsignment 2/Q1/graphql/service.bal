...
mysql:Client management = check new("localhost", "root", "root", "assignment-2-p1", 3307);
...
service /graphql on new graphql:Listener(8080) {
    ...
    resource function get user(int id, string token) returns User|error {
        ...
        stream<User, sql:Error?> resultStream = management->query(query, User);
    ...
    resource function get department(int id, string token) returns Department|error {
        ...
        stream<Department, sql:Error?> depStream = management->query(depQuery);
        ...
        if (depResult is record {| Department value; |}) {
            ...
            if (dept.hodId is int) {
                ...
                stream<User, sql:Error?> hodStream = management->query(hodQuery, User);
    ...
    resource function get departmentObjective(int id, string token) returns DepartmentObjective|error {
        ...
        stream<DepartmentObjective, sql:Error?> objStream = management->query(objQuery);
        ...
        if (objResult is record {| DepartmentObjective value; |}) {
            ...
            stream<KPI, sql:Error?> kpiStream = management->query(kpiQuery);
            KPI[] kpis = [];
    ...
    resource function get users(string token) returns User[]|error {
        ...
        stream<User, sql:Error?> userStream = management->query(userQuery);
        User[] users = [];
    ...
    resource function get createUser(string firstName, string lastName, string jobTitle, string position, UserRole role, int departmentId, string token) returns User|error {
        ...
        var response = management->execute(query);
...
resource function get login(string firstName, string lastName, string email, string password) returns string|error {
    ...
    stream<record {| string salt; |}, sql:Error?> saltStream = management->query(saltQuery);
    ...
    if (saltResult is record {| record {| string salt; |} value; |}) {
        ...
        stream<record {| int id; string firstName; string lastName; string? jobTitle; string? position; string role; int? departmentId; string email; string hashedPassword; string salt; |}, sql:Error?> resultStream = management->query(authQuery);
    ...
    resource function get register(string firstName, string lastName, string email, string password, string jobTitle, string position, string role, int departmentId) returns string|error {
        ...
        var userInsertResponse = management->execute(insertUserQuery);
        if (userInsertResponse is sql:ExecutionResult) {
            ...
            if (userId is int) {
                ...
                var authInsertResponse = management->execute(authQuery);
...
// jdbc:Client management = check new ("jdbc:mysql://localhost:3307/assignment-2-p1", "root", "root");
